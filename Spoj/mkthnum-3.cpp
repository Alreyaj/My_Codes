...
int qry(node *pr, node *n, int b, int e, int pos)
{
    if(b == e) return b;
    int co = n->left->val - pr->left->val;
    int m = (b + e) >> 1;
    if(co >= pos)
        return qry(pr->left, n->left, b, m, pos);
    else
        return qry(pr->right, n->right, m + 1, e, pos - co);
}
...
int main()
{
    ...
    {
        scanf("%d %d %d", &x, &y, &val);
		printf("%d\n", cn[qry(version[x - 1], version[y], 1, n, val)]);
    }
}
